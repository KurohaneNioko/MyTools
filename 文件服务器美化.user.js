...
// @namespace    Nioko
// @version      0.1
// @description  下载服务器美化调整：全文件名显示 播放列表导出 大小按照K/M/G显示
// @author       You
...
(function() {
    ...
    if(!(current_url.indexOf('/qb/')>-1 || current_url.indexOf('/aria2/')>-1)){
        function getLen(str){
    ...
    if(!(current_url.indexOf('/qb/')>-1 || current_url.indexOf('/aria2/')>-1)){
        var hr = "https://www.thiswaifudoesnotexist.net/example-"+rnd_int.toString()+'.jpg';
        $('#ico-origin').click(function(){
            window.open(hr, '_blank');
        });
        $('#full-ico').attr('src', link.href);
    }
